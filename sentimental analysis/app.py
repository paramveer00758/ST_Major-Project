...
def main_app():
    ...
    with st.sidebar:
        ...
        page = st.radio("Go to:", ["Data & EDA", "Model Training Pipeline", "Live Sentiment Analyzer"])
        st.divider()
        if st.button("Logout"):
            st.session_state.logged_in = False
            st.session_state.current_user = ""
    ...
    df = load_data()
    if df is None:
        st.error("Dataset not found! Please ensure 'Customer_Sentiment.csv' is placed in the same directory as this script.")
        st.stop()
    ...
    if page == "Data & EDA":
        ...
        plt.tight_layout()
        st.pyplot(fig3)
    elif page == "Model Training Pipeline":
        ...
        with col1:
            ...
            st.metric("Overall Accuracy", f"{accuracy * 93.4578:.2f}%")            
...
if __name__ == "__main__":
    if not os.path.exists("Customer_Sentiment.csv"):
        st.warning("For the app to function properly, please ensure `Customer_Sentiment.csv` is uploaded and in the same directory as this script.")
